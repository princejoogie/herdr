...
Run best-effort commands during worktree creation or removal with lifecycle hooks:
...
Available phases are `before_create`, `after_create`, `before_remove`, and `after_remove`. Commands are argv arrays and do not use a shell unless one is explicitly included. Create hooks run from the source checkout before creation and the new checkout afterward. Remove hooks run from the checkout before removal and the repository root afterward.

Hooks receive `HERDR_WORKTREE_HOOK`, `HERDR_WORKTREE_PATH`, `HERDR_WORKTREE_REPO_ROOT`, `HERDR_WORKTREE_SOURCE_PATH`, and, when available, `HERDR_WORKTREE_BRANCH`. A missing command, non-zero exit, or other hook failure is logged and shown as a toast, but never fails or rolls back the worktree operation. Commands in the same phase run serially in declaration order.
